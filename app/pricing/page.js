...
const Pricing = () => {
  ...
  return (
    <div className="relative min-h-screen bg-black bg-opacity-40 font-poppins">
      ...
      />
      <div className="absolute w-full h-[calc(100%-320px)]">
        <Image
      ...
      <div className="relative flex flex-col items-center justify-center px-4 sm:px-6 lg:px-8 pb-16">
        {/* Adjusted padding bottom here */}
        <div className="text-white mt-6 space-y-6 w-full max-w-8xl">
          ...
          <div className="bg-[#484848] bg-opacity-[0.36] p-6 rounded-lg border border-[#A6A6A6] mt-10 w-full text-left">
            <p className="text-xl font-extrabold">Want to Distribute a Single Track?</p>
            <p className="text-sm">Upload one song to major platforms for just ₹399</p>
            <div className="flex justify-between items-center">
              <div>
               
                <p className="text-xs text-opacity-10 mt-8 text-gray-400">(Note: This plan does not include access to the dashboard)</p>
              </div>
    ...
    <div>
      ...
      {slider && (
        <div className="flex justify-between items-center mt-1">
          <span>Artists No: {slider.value}</span> {/* Added label "Artists No:" */}
          <input
            type="range"
            min={slider.min}
            max={slider.max}
      ...
      <p className="text-sm text-gray-400 mt-8">{description}</p> {/* Changed description color to gray */}
